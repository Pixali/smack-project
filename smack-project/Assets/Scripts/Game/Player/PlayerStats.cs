...
namespace Player {
    public class PlayerStats : IStatSource {
        ...
        public PlayerStats() {
            //BasicStats = JsonConvert.DeserializeObject<StatBundle>(File.ReadAllText("Assets/Scripts/Data/BasicPlayerStats.json"));
            BasicStats = JsonConvert.DeserializeObject<StatBundle>(Resources.Load<TextAsset>("Data/BasicPlayerStats").text); // needed so game can find files correctly on export
